        ...
        >
          <div className="relative size-5 shrink-0">
            <div className="absolute inset-0 flex size-5 items-center justify-center overflow-hidden rounded bg-background transition-opacity group-hover:opacity-0">
              {isImage ? (
                // eslint-disable-next-line @next/next/no-img-element -- Dynamic data URL from attachment
                <img
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = (event) => {
    ...
    Promise.all(
      files.map(async ({ id: _id, ...item }) => {
        void _id; // id is destructured out but not needed for conversion
...
// Check for speech recognition support at module level (runs once)
