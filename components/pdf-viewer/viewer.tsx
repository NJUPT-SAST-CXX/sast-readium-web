...
import { PDFToolbar } from "./toolbar/toolbar";
import { PDFMobileToolbar } from "./toolbar/mobile-toolbar";
import { PDFTTSReader } from "./tts-reader";
import { PDFPage } from "./page";
import { PasswordDialog } from "./dialogs/password";
import { PDFDraggableThumbnail } from "./navigation/draggable-thumbnail";
import { PDFOutline } from "./navigation/outline";
import { PDFBookmarks } from "./navigation/bookmarks";
import { PDFAnnotationsList } from "./annotations/annotations-list";
import { PDFProgressBar } from "./navigation/progress-bar";
import { PDFTextLayer } from "./layers/text-layer";
import { PDFAnnotationLayer } from "./annotations/annotation-layer";
import { PDFDrawingLayer } from "./layers/drawing-layer";
import { PDFSelectionLayer } from "./layers/selection-layer";
import { KeyboardShortcutsDialog } from "./dialogs/keyboard-shortcuts";
import { PDFContextMenu } from "./toolbar/context-menu";
...
import { SignatureDialog } from "./dialogs/signature";
...
export function PDFViewer({
  ...
  }, [isLoading]); // Re-run when loading completes to attach listeners after container is rendered
  ...
  return (
    ...
    >
      ...
      {!isPresentationMode && (
        <div
          className={cn("flex flex-1 overflow-hidden", contentBottomPadding)}
        >
          <div
            className={cn(
                    ...
                    {(pageOrder.length > 0
                      ...
                      return (
                        <PDFDraggableThumbnail
                          ...
                          onRemove={() => {
                            if (
                              window.confirm(t("viewer.confirm_delete_page"))
                            ) {
                              removePage(i);
                            }
                          }}
          ...
          <div
            ...
            className="flex-1 overflow-auto bg-muted/50 custom-scrollbar-wide"
            style={{
                    ...
                    >
                      <div className="relative">
                        <PDFPage
                          ...
                          className="shadow-lg"
                          width={
                            dimensions ? dimensions.width * zoom : undefined
                          }
                          height={
              ...
              <div className="flex flex-col items-center gap-4 p-8">
                {Array.from({ length: Math.ceil(numPages / 2) }).map(
                  (_, pairIndex) => {
                    ...
                    const leftDim =
                      pageDimensionsRef.current.get(leftPageNumber);
                    const rightDim =
                          ...
                          >
                            <div className="relative">
                              <PDFPage
                                ...
                                className="shadow-lg"
                                width={
                                  leftDim ? leftDim.width * zoom : undefined
                                }
                                height={
