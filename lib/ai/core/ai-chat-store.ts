...
export const BUILTIN_QUICK_COMMANDS: Omit<QuickCommand, "id">[] = [
  {
    ...
    prompt:
      "Please provide a comprehensive summary of the following content from page {{page}}:\n\n{{selection}}",
    icon: "FileText",
    category: "builtin",
    shortcut: "/sum",
    enabled: true,
  },
  {
    ...
    prompt:
      "Please translate the following text to {{language}}. Preserve formatting where possible:\n\n{{selection}}",
    icon: "Languages",
    category: "builtin",
    shortcut: "/tr",
    enabled: true,
  ...
  },
  {
    ...
    prompt:
      "Please extract and list the key points from the following content. Use bullet points for clarity:\n\n{{selection}}",
    icon: "List",
    category: "builtin",
    shortcut: "/key",
    enabled: true,
  },
  {
    ...
    prompt:
      "Based on the following content from page {{page}}, generate 5-10 study questions that would help understand the material:\n\n{{selection}}",
    icon: "HelpCircle",
    category: "builtin",
    shortcut: "/q",
    enabled: true,
  },
  {
    ...
    prompt:
      "Please provide clear definitions and explanations for the key terms and concepts in the following text:\n\n{{selection}}",
    icon: "BookOpen",
    category: "builtin",
    shortcut: "/def",
    enabled: true,
  },
  {
    ...
    prompt:
      "Please simplify the following text, making it easier to understand while preserving the key information:\n\n{{selection}}",
    icon: "Sparkles",
    category: "builtin",
    shortcut: "/simple",
    enabled: true,
  },
  {
    ...
    prompt:
      "Please identify and compare the different concepts, ideas, or items mentioned in the following text:\n\n{{selection}}",
    icon: "GitCompare",
    category: "builtin",
    shortcut: "/cmp",
    enabled: true,
...
export const BUILTIN_PROMPT_TEMPLATES: Omit<
  PromptTemplate,
...
>[] = [
  {
    ...
    content:
      'Please provide an academic summary of the following content from "{{fileName}}" (Page {{page}}):\n\n{{selection}}\n\nInclude:\n1. Main thesis/argument\n2. Key supporting points\n3. Methodology (if applicable)\n4. Conclusions\n5. Implications',
    category: "Academic",
    isBuiltin: true,
  },
  {
    ...
    content:
      "Please analyze the following code from the document:\n\n{{selection}}\n\nProvide:\n1. What the code does\n2. Key functions/methods\n3. Potential improvements\n4. Any issues or bugs",
    category: "Technical",
    isBuiltin: true,
  },
  {
    ...
    content:
      "Please extract meeting notes from the following content:\n\n{{selection}}\n\nFormat as:\n- **Attendees**: (if mentioned)\n- **Key Discussion Points**:\n- **Decisions Made**:\n- **Action Items**:\n- **Next Steps**:",
    category: "Business",
    isBuiltin: true,
  },
  {
  ...
  },
  {
    ...
    content:
      "Please explain the following content as if you were explaining it to a 5-year-old. Use simple words, analogies, and examples:\n\n{{selection}}",
    category: "General",
    isBuiltin: true,
  },
  {
...
interface AIChatState {
  ...
  getAllModels: () => (
    | AIModel
    | (CustomAIModel & { providerId: string; providerName: string })
  )[];
}
    ...
    (set, get) => ({
      ...
      createConversation: (title) => {
        ...
        const conversation: Conversation = {
          ...
          title:
            title ||
            (pdfContext ? `Chat: ${pdfContext.fileName}` : "New Conversation"),
          messages: [],
          createdAt: Date.now(),
          updatedAt: Date.now(),
        ...
        set((state) => {
          ...
          return {
            conversations: {
              ...
              [currentConversationId]: {
                ...conversations[currentConversationId],
                messages: conversations[currentConversationId].messages.map(
      ...
      updateSessionUsage: (usage) =>
        set((state) => {
          const newSessionUsage = {
            totalInputTokens:
              state.sessionUsage.totalInputTokens + (usage.inputTokens ?? 0),
            totalOutputTokens:
              state.sessionUsage.totalOutputTokens + (usage.outputTokens ?? 0),
            totalReasoningTokens:
              state.sessionUsage.totalReasoningTokens +
              (usage.reasoningTokens ?? 0),
            totalCachedTokens:
              state.sessionUsage.totalCachedTokens +
            ...
            return {
              ...
              conversations: {
                ...
                [currentConversationId]: {
                  ...
                  usage: {
                    totalInputTokens:
                      convUsage.totalInputTokens + (usage.inputTokens ?? 0),
                    totalOutputTokens:
                      convUsage.totalOutputTokens + (usage.outputTokens ?? 0),
                    totalReasoningTokens:
                      convUsage.totalReasoningTokens +
                      (usage.reasoningTokens ?? 0),
                    totalCachedTokens:
                      convUsage.totalCachedTokens +
                      (usage.cachedInputTokens ?? 0),
      ...
      deleteCustomProvider: (id) =>
        set((state) => ({
          settings: {
            ...state.settings,
            customProviders: state.settings.customProviders.filter(
              (p) => p.id !== id
            ),
        ...
        set((state) => ({
          settings: {
            ...
            transcriptionSettings: {
              ...state.settings.transcriptionSettings,
              ...settings,
            },
          },
        })),
      ...
      deleteQuickCommand: (id) =>
        set((state) => ({
          settings: {
            ...state.settings,
            quickCommands: state.settings.quickCommands.filter(
              (cmd) => cmd.id !== id
            ),
          },
        })),
      ...
      deletePromptTemplate: (id) =>
        set((state) => ({
          settings: {
            ...state.settings,
            promptTemplates: state.settings.promptTemplates.filter(
              (tpl) => tpl.id !== id
            ),
          },
        })),
      ...
      getPromptTemplatesByCategory: (category) => {
        const { settings } = get();
        return settings.promptTemplates.filter(
      ...
      processTemplate: (template, extraVars = {}) => {
        ...
        result = result.replace(
          /\{\{page\}\}/g,
          String(pdfContext?.currentPage || 1)
        );
        result = result.replace(
          /\{\{pageText\}\}/g,
          pdfContext?.pageText || "[No page text available]"
        );
        result = result.replace(
          /\{\{fileName\}\}/g,
          pdfContext?.fileName || "Document"
        );
        result = result.replace(
          /\{\{totalPages\}\}/g,
          String(pdfContext?.totalPages || 1)
        );
        result = result.replace(
          /\{\{language\}\}/g,
          extraVars.language || settings.defaultTranslationLanguage || "Chinese"
        );
      ...
      merge: (persistedState, currentState) => {
        ...
        return {
          ...
          settings: {
            ...
            enableMultiStepTools:
              persisted?.settings?.enableMultiStepTools ?? true,
            maxToolSteps: persisted?.settings?.maxToolSteps ?? 5,
            // Ensure media settings have defaults for backward compatibility
            imageSettings:
              persisted?.settings?.imageSettings || DEFAULT_IMAGE_SETTINGS,
            speechSettings:
              persisted?.settings?.speechSettings || DEFAULT_SPEECH_SETTINGS,
            transcriptionSettings:
              persisted?.settings?.transcriptionSettings ||
            ...
            defaultTranslationLanguage:
              persisted?.settings?.defaultTranslationLanguage || "Chinese",
          },
        };
      },
    }
