...
async function processZipArchive(file: File): Promise<File[]> {
  ...
  for (const key of Object.keys(zip.files)) {
    ...
    if (lowerKey.endsWith('.pdf')) {
      ...
      const extractedFile = new File([blob], key.split('/').pop() ?? key, { type: 'application/pdf' });
...
async function processRarArchive(file: File): Promise<File[]> {
  ...
  for (const file of extracted.files) {
    ...
    if (lowerKey.endsWith('.pdf')) {
      if (file.extraction) {
        const blob = new Blob([file.extraction as unknown as BlobPart], { type: 'application/pdf' });
        const extractedFile = new File([blob], key.split('/').pop() ?? key, { type: 'application/pdf' });
        pdfFiles.push(extractedFile);
      }
    } else {
...
async function convertImagesToPdf(
  ...
  const newFileName = originalFileName.replace(/\.(zip|cbz|rar|cbr)$/i, '') + '.pdf';
  
  // Fix type error by casting or using Blob
  return new File([pdfBytes as BlobPart], newFileName, { type: 'application/pdf' });
}
