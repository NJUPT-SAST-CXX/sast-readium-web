...
/**
 * Metadata for AI-generated annotations
 * Tracks the source and context of AI-created annotations
 */
export interface AnnotationMetadata {
  /** Source of the annotation: 'user' for manual, 'ai' for AI-generated */
  source: "user" | "ai";
  /** AI message ID that created this annotation */
  aiMessageId?: string;
  /** AI conversation ID for reference */
  aiConversationId?: string;
  /** AI model used to generate the annotation */
  aiModel?: string;
  /** User query that triggered the AI annotation */
  aiQuery?: string;
  /** AI confidence score (0-1) */
  confidence?: number;
  /** AI's reasoning for the annotation */
  reasoning?: string;
  /** IDs of related annotations */
...
export interface Annotation {
  ...
  strokeWidth?: number;
  timestamp: number;
  /** Optional metadata for AI-generated annotations */
...
interface PDFState {
  ...
  addAIAnnotation: (
    annotation: Omit<Annotation, "id" | "timestamp" | "metadata">,
    aiContext: {
      messageId: string;
  ...
  getAIAnnotations: () => Annotation[];
  getAnnotationsBySource: (source: "user" | "ai") => Annotation[];
  undoAnnotation: () => void;
  redoAnnotation: () => void;
  canUndo: () => boolean;
  canRedo: () => boolean;
...
export const usePDFStore = create<PDFState>()(
  persist(
    (set, get) => ({
      ...
      addAIAnnotation: (annotation, aiContext) => {
        const newAnnotation: Annotation = {
          ...annotation,
          id: `ai-annotation-${Date.now()}-${Math.random()}`,
          timestamp: Date.now(),
          metadata: {
            source: "ai",
            aiMessageId: aiContext.messageId,
            aiConversationId: aiContext.conversationId,
            aiModel: aiContext.model,
            aiQuery: aiContext.query,
      ...
      getAIAnnotations: () => {
        const state = get();
        return state.annotations.filter((a) => a.metadata?.source === "ai");
      },
      ...
      getAnnotationsBySource: (source) => {
        ...
        // User annotations are those without metadata or with metadata.source === 'user'
        return state.annotations.filter(
          (a) => !a.metadata || a.metadata.source === "user"
        );
      },
      ...
      exportAnnotations: () => {
        ...
        const exportData = {
          ...
          version: "1.1", // Updated version for AI annotation metadata support
        };
